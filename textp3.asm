...
play		macro	divisor, octave, duration
		mov	ax, divisor ; getting the divisor value
		mov	duration1, duration ; getting the duration
		mov bx, octave ; getting the octave
		call Note
		endm
stop		macro	duration
local Sloop
...
delay	cx ;using the delay procedure
...
local notasong
mov ah, 3dh 
xor al, al ; I want to only read the file, without editing it
...
cmp cx, 0FFh
je notasong
xor di, di ;di will be used for memory access, so I have to reset it
songloop:
call songproc
loop songloop
notasong:
...
mov ah, 2ch
		...
		xor dh, dh ;I want the miliseconds, which are in dl. Therefore, I must reset dh in order to be able to use dx as dl.
		mov timesaver, dx ;saving the miliseconds
		loopb:
		int 21h ;getting the time again
		xor dh, dh ;make sure that only the miliseconds are in dx
...
buffer byte ?
db 65075 dup(0); the space where the songs will be stored
dseg		ends
...
cmp bx, 10h ; if the notes are above 9, they are represented as letters(A, B, C), and when you turn them into a number, you get 11h, 12h and 13h and not A, B and C, so I must decrease them by 7 if they are above 10h in order to get the correct note
...
xor ch, ch ; I need to use cx, so I must reset ch
...
play ax, bx, dx ; call the play macro. ax contain the note, bx contain the octave and dx contain the duration
pop di ;restore the previous value
inc di ;move to the next note
pop cx ;return the first value of cx(that belong to songloop)
	Ret
songproc EndP
rpal proc
mov si, 167 ;si is the pointer, and it's located after the header.
mov cx, 256 ;there are 255 colors in the pallete
mov dx, 3C8h
xor al, al
...
shr     al,2
out     dx,al                   ; Send it.
add     si,4                    ; move to the next colors.
loop    sndLoop
    Ret
rpal EndP
paint proc
mov cx, 32000 ;there are 64K pixels in the photo. I'm reading 2 at a time, so I only have to do it 32000 times.
mov si, 1190 ;moving the starting position into si
xor di, di ;moving the starting position in the video memory into di
mov ax, 0A000h 
...
delay	cx ; Delaying the program until the note has done playing
...
delay 1; reset the clock so the notes will be in the correct length
mov cx, 0FFh
xor si, si
clean:
mov buffer[si], 0
inc si
loop clean ;cleaning the place where the songs will be stored from the photo
mov ah, 07h
